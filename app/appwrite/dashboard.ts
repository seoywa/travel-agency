...
export const getUserGrowthPerDay = async () => {
  ...
  const userGrowth = users.documents.reduce(
    (acc: { [key: string]: number }, user: Document) => {
      //-It looks at their joinedAt date to determine when they signed up
      const date = new Date(user.joinedAt);
      const day = date.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric'
      });
      //- Count Daily Sign-ups: It groups the users by their sign-up date and counts how many joined on each day.
      acc[day] = (acc[day] || 0) + 1
      return acc;
    },
    {}
...
export const getTripsByTravelStyle = async () => {
  ...
  const travelStyleCount = trips.documents.reduce(
    (acc: {[key: string]: number}, trip: Document) => {
  ...
  return Object.entries(travelStyleCount).map(([travelStyle, count]) => ({
    count: Number(count),
    travelStyle
  }))
}